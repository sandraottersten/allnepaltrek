...
<header
	class="fixed left-0 z-50 hidden w-full justify-between duration-300 md:flex {lastScrollTop > vh
		? 'bg-light80 text-dark shadow-xl backdrop-blur-xl'
		: ' bg-dark30 text-light'}
    {scrollDown ? 'translate-y-[-100%] transform' : 'translate-y-0 transform'}"
>
	<a
		href="/"
		class="flex h-[80px] items-center rounded-br-lg pl-10 pr-10 {lastScrollTop > vh
			? 'bg-dark80'
			: 'bg-dark80'}"
	>
		<Logo
			size={lastScrollTop > vh ? 'w-[80px]' : 'w-[80px]'}
			textColor={lastScrollTop > vh ? '#F8F8F8' : '#F8F8F8'}
		/></a
	>
	<div class="flex w-full flex-col">
		<div class="flex h-8 w-full items-center justify-end bg-dark80 px-10">
			<span class="flex items-center text-sm text-light">
				Bhim Guide <span class="ml-3 mr-2"><WhatsApp size="size-[20px]" color="#FFFFFF" /></span> +9779841356471
			</span>
		</div>
											...
											>
												...
												<ChevronRight
													...
													class={i > 0 && subItem.subItems
														? 'stroke-dark30 group-hover/sub:stroke-orange'
														: 'opacity-0'}
												/>
											</a>
											{#if subItem.subItems}
												<ul
													class="absolute -top-1 left-[100%] hidden flex-col rounded-lg bg-light py-1 text-dark shadow-xl backdrop-blur-xl group-hover/sub:flex"
												>
